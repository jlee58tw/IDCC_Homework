...
def mail_title(title,url,time):
        # send mail
        for sub in subscrs:
                s = re.search(sub[3], title, re.IGNORECASE)
                #compare singer name with raw title
                #print title, sub[3]
                if s is not None:
                        singername = sub[3]
                       ...
                       #send the mail
                        print "sent"
                        sendmail(singername,receiver,title,url,time)
                else:
                        pass
        #find nothing 
